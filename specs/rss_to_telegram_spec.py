...
with description('App rss_to_telegram', 'unit') as self:
    with context('running the service'):
        with before.each:
            ...
            self.birthday_service = Spy(BirthdayService)
            ...
            self.a_birthday_publication = object_mother.a_publication(date=self.a_datetime)
            ...
            self.send_new_entries_use_case = SendNewPublications(self.my_logger, self.last_entry_service, self.twitter_publication_service,
                                                                 self.blog_publication_service, self.last_entry_datetimes, self.telegram_notifier_service, self.board_message_publication_service, self.last_board_message_service, self.birthday_service)
            ...
            when(self.birthday_service).get_new_publications(ANY_ARG).returns(([self.a_birthday_publication]))
        ...
        with it('retrieves new birthday messages'):
            self.send_new_entries_use_case.send()

            expect(self.birthday_service.get_new_publications).to(have_been_called_with())

        with it('sends to telegram new publications'):
            self.send_new_entries_use_case.send()

            expect(self.telegram_notifier_service.send_publications).to(have_been_called_with([self.a_feed_publication, self.a_twitter_publication, self.a_board_publication, self.a_birthday_publication]))
